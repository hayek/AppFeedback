...
final class MailThreadStoreTests: XCTestCase {
    ...
    // MARK: - Test 6: mergeThreads reparents messages and deletes dropped thread

    func test_mergeThreads_reparentsMessagesAndDeletesDrop() throws {
        let context = try makeContext()
        let store = MailThreadStore(context: context)

        // Create two separate threads
        let msg1 = store.recordOutbound(
            messageID: "<thread-a-msg1@example.com>",
            repoOwner: "owner", repoName: "repo", issueNumber: 1,
            from: "a@example.com", fromName: nil,
            to: ["b@example.com"], cc: [],
            subject: "Thread A",
            bodyPlain: "A1", bodyHTML: nil,
            date: Date(timeIntervalSinceNow: -300),
            replyHeaders: nil
        )
        let msg2 = store.recordOutbound(
            messageID: "<thread-b-msg1@example.com>",
            repoOwner: "owner", repoName: "repo", issueNumber: 2,
            from: "c@example.com", fromName: nil,
            to: ["d@example.com"], cc: [],
            subject: "Thread B",
            bodyPlain: "B1", bodyHTML: nil,
            date: Date(timeIntervalSinceNow: -200),
            replyHeaders: nil
        )

        let keepThread = msg1.thread!
        let dropThread = msg2.thread!

        store.mergeThreads(into: keepThread, drop: dropThread)

        // keepThread should now have both messages
        let keepMessages = keepThread.messages
        XCTAssertEqual(keepMessages.count, 2, "Keep thread should have 2 messages after merge")

        // dropThread should be deleted
        let threads = try context.fetch(FetchDescriptor<MailThread>())
        XCTAssertEqual(threads.count, 1, "Only 1 thread should remain")
        XCTAssertEqual(threads.first?.id, keepThread.id)
    }

    // MARK: - Test 7: mergeThreads skips reparenting messages with duplicate messageID

    func test_mergeThreads_skipsDuplicateMessageIDsDuringMerge() throws {
        let context = try makeContext()
        let store = MailThreadStore(context: context)

        let sharedMsgID = "<shared@example.com>"
        ...
        // (bypass recordOutbound dedupe by inserting directly)
        let dropThread = MailThread(
            messageIDRoot: "<drop-root@example.com>",
            subject: "Drop Thread"
        )
        context.insert(dropThread)

        let duplicateMsg = MailMessage(
            messageID: sharedMsgID,  // same as the one already in keepThread
            subject: "Shared",
        ...
        // keepThread should still have only 1 message (the duplicate was skipped)
        XCTAssertEqual(keepThread.messages.count, 1, "Duplicate message should be dropped, not reparented")

        let threads = try context.fetch(FetchDescriptor<MailThread>())
        XCTAssertEqual(threads.count, 1, "Only keep thread should remain")
    }

    // MARK: - Test 8: attachToIssue updates the three denormalized fields

    func test_attachToIssue_updatesIssueFields() throws {
        let context = try makeContext()
        let store = MailThreadStore(context: context)

        // Create orphan thread (no issue link)
        let msg = store.recordInbound(message: makeParsed(
            messageID: "<orphan2@example.com>"
        ))
        let thread = msg?.thread
        XCTAssertNotNil(thread)
        XCTAssertEqual(thread?.issueNumber, 0)

        store.attachToIssue(thread: thread!, owner: "myorg", repo: "myrepo", number: 99)

        XCTAssertEqual(thread?.issueRepoOwner, "myorg")
        XCTAssertEqual(thread?.issueRepoName, "myrepo")
        XCTAssertEqual(thread?.issueNumber, 99)

        // Verify persisted
        let threads = try context.fetch(FetchDescriptor<MailThread>())
        let persisted = threads.first(where: { $0.id == thread?.id })
        XCTAssertEqual(persisted?.issueNumber, 99)
    }
